...
default_3D_augmentation_params = {
    ...
    "do_rotation": True,
    "rotation_x": (-15. / 360 * 2. * math.pi, 15. / 360 * 2. * math.pi),
    "rotation_y": (-15. / 360 * 2. * math.pi, 15. / 360 * 2. * math.pi),
    "rotation_z": (-15. / 360 * 2. * math.pi, 15. / 360 * 2. * math.pi),
...
default_2D_augmentation_params["rotation_x"] = (-180. / 360 * 2. * math.pi, 180. / 360 * 2. * math.pi)
default_2D_augmentation_params["rotation_y"] = (-0. / 360 * 2. * math.pi, 0. / 360 * 2. * math.pi)
default_2D_augmentation_params["rotation_z"] = (-0. / 360 * 2. * math.pi, 0. / 360 * 2. * math.pi)
...
def rotate_coords_3d(coords, angle_x, angle_y, angle_z):
    ...
    coords = tf.matmul(tf.transpose(tf.reshape(coords, (len(coords), -1))), rot_matrix).transpose()
...
def create_matrix_rotation_y_3d(angle, matrix=None):
    rotation_y = tf.constant([[tf.math.cos(angle), 0, tf.math.sin(angle)],
                        [0, 1, 0],
...
def create_matrix_rotation_z_3d(angle, matrix=None):
    rotation_z = tf.constant([[tf.math.cos(angle), -tf.math.sin(angle), 0],
...
def create_matrix_rotation_2d(angle, matrix=None):
    rotation = tf.constant([[tf.math.cos(angle), -math.sin(angle)],
...
def get_batch_size(final_patch_size, rot_x, rot_y, rot_z, scale_range):
    ...
    rot_x = min(90 / 360 * 2. * tf.pi, rot_x)
    rot_y = min(90 / 360 * 2. * tf.pi, rot_y)
    rot_z = min(90 / 360 * 2. * tf.pi, rot_z)
    coords = tf.constant(final_patch_size)
    final_shape = tf.identity(coords)
    if len(coords) == 3:
        final_shape = tf.math.reduce_max(tf.concat((tf.abs(rotate_coords_3d(coords, rot_x, 0, 0)), final_shape)), axis=0)
        final_shape = tf.math.reduce_max(tf.concat((tf.abs(rotate_coords_3d(coords, 0, rot_y, 0)), final_shape)), axis=0)
        final_shape = tf.math.reduce_max(tf.concat((tf.abs(rotate_coords_3d(coords, 0, 0, rot_z)), final_shape)), axis=0)
    elif len(coords) == 2:
        final_shape = tf.math.reduce_max(tf.concat((tf.abs(rotate_coords_2d(coords, rot_x)), final_shape)), axis=0)
...
class DataAugmentor():
    ...
    def process_plans(self, plans):
        ...
        self.patch_size = stage_plans['patch_size']  # here, in the orginal code, the author convert it to np.array. It may need to change later.
    ...
    def setup_DA_params(self):
        if self.threeD:
            ...
            self.data_aug_param['rotation_x'] = (-30. / 360 * 2. * math.pi, 30. / 360 * 2. * math.pi)
            self.data_aug_param['rotation_y'] = (-30. / 360 * 2. * math.pi, 30. / 360 * 2. * math.pi)
            self.data_aug_param['rotation_z'] = (-30. / 360 * 2. * math.pi, 30. / 360 * 2. * math.pi)
            if self.do_dummy_2D_aug:
                self.data_aug_param['dummy_2D'] = True
                print('Using dummy2d data augmentation')
                self.data_aug_param['elastic_deform_alpha'] = \
                    default_2D_augmentation_params['elastic_deform_alpha']
                self.data_aug_param['elastic_deform_sigma'] = \
                    default_2D_augmentation_params['elastic_deform_sigma']
                self.data_aug_param['rotation_x'] = default_2D_augmentation_params['rotation_x']
        else:
            self.do_dummy_2D_aug = False
            if max(self.patch_size) / min(self.patch_size) > 1.5:
                default_2D_augmentation_params['rotation_x'] = (-15. / 360 * 2. * math.pi, 15. / 360 * 2. * math.pi)
            self.data_aug_param = default_2D_augmentation_params
        self.data_aug_param['mask_was_used_for_normalization'] = self.use_mask_for_norm

        if self.do_dummy_2D_aug:
            self.basic_generator_patch_size = get_batch_size(self.patch_size[1:],
            ...
            self.basic_generator_patch_size = tf.constant([self.patch_size[0]] + list(self.basic_generator_patch_size))
            assert self is None, f'do dummpy 2d: {self.basic_generator_patch_size}'

        else:
            self.basic_generator_patch_size = get_batch_size(self.patch_size,
                                                            self.data_aug_param['rotation_x'],
                                                            self.data_aug_param['rotation_y'],
                                                            self.data_aug_param['rotation_z'],
                                                            self.data_aug_param['scale_range'])
        ...
        self.data_aug_param['patch_size_for_spatial_transform'] = self.patch_size
        self.data_aug_param['num_cached_per_thread'] = 2

    def formalize_data(self, case_identifier, image, label, class_locations_bytes, class_locations_shape, return_type):
        ...
        for i, class_locations_decode in enumerate(class_locations_bytes):
            class_locations_decode = tf.io.decode_raw(class_locations_decode, tf.int64)
            class_locations[tf.constant(i+1, dtype=tf.int32)] = tf.reshape(class_locations_decode, (class_locations_shape[i], -1))
        need_to_pad = tf.convert_to_tensor(self.basic_generator_patch_size, dtype=tf.int64) - tf.convert_to_tensor(self.patch_size, dtype=tf.int64)
        ...
        for d in range(3):
            if need_to_pad[d] + case_all_data.shape[d+1] < self.basic_generator_patch_size[d]:
                need_to_pad_d = self.basic_generator_patch_size[d] - case_all_data.shape[d+1]
        ...
        if not force_fg:
            ...
            bbox_z_lb = tf.random.uniform([], minval=lb_z, maxval=ub_z+1, dtype=tf.int64)
        else:
            foreground_classes = tf.constant([i for i in class_locations.keys() if i != 0])
            if len(foreground_classes) == 0:
                selected_class = None
                voxels_of_that_class = None
                print(f'case does not contain any foreground classes {case_identifier}')
            else:
            ...
            if voxels_of_that_class is not None:
                ...
                bbox_x_lb = tf.random.uniform([], minval=lb_x, maxval=ub_x+1, dtype=tf.int32)
                bbox_y_lb = tf.random.uniform([], minval=lb_y, maxval=ub_y+1, dtype=tf.int32)
                bbox_z_lb = tf.random.uniform([], minval=lb_z, maxval=ub_z+1, dtype=tf.int32)
        ...
        '''
        image = tf.pad(case_all_data[:-1], ([0, 0],
                                            [-min(0, bbox_x_lb), max(bbox_x_ub - shape[0], 0)],
                                            [-min(0, bbox_y_lb), max(bbox_y_ub - shape[1], 0)],
                                            [-min(0, bbox_z_lb), max(bbox_z_ub - shape[2], 0)]),
                    mode='CONSTANT')
        seg = tf.pad(case_all_data[-1:], ([0, 0],
                                        [-min(0, bbox_x_lb), max(bbox_x_ub - shape[0], 0)],
                                        [-min(0, bbox_y_lb), max(bbox_y_ub - shape[1], 0)],
                                        [-min(0, bbox_z_lb), max(bbox_z_ub - shape[2], 0)]),
    ...
    def formalize_data_3d(self, data):
        case_identifier, image_raw, label_raw, class_locations_bytes, class_locations_shape = data['case_identifier'], data['image/encoded'], data['image/class/label'], data['class_locations_bytes'], data['class_locations_shape']
        ...
        for i in range(self.batch_size):
            ...
            original_image, original_label = tf.reshape(image, original_image_size[i]), tf.reshape(label, original_label_size[i])
            ...
            for c in range(class_locations_bytes[i].shape[0]):
                class_locations_decode = tf.io.decode_raw(class_locations_bytes[i][c], tf.int64)
                class_locations[c+1] = tf.reshape(class_locations_decode, [class_locations_shape[i][c], -1])
            if self.get_do_oversample(i):
                force_fg = True
            else:
                force_fg = False
            case_all_data = tf.concat([image, label], axis=0)
            self.basic_generator_patch_size = tf.convert_to_tensor(self.basic_generator_patch_size, dtype=tf.int64)
            self.patch_size = tf.convert_to_tensor(self.patch_size, dtype=tf.int64)
            need_to_pad = self.basic_generator_patch_size - self.patch_size
            for d in range(3):
                if need_to_pad[d] + tf.shape(case_all_data, out_type=tf.int64)[d+1] < self.basic_generator_patch_size[d]:
            ...
            if not force_fg:
                ...
                foreground_classes = [c for c in class_locations.keys() if c != 0]
                if len(foreground_classes) == 0:
                    selected_class = None
                    voxels_of_that_class = None
                    tf.print(f'case does not contain any foreground classes {case_identifier}')
                else:
                    voxels_of_that_class = class_locations[1]
                    selected_voxel = voxels_of_that_class[0]

                    rand_val = tf.random.uniform([], minval=0, maxval=len(class_locations), dtype=tf.int32)
                ...
                if voxels_of_that_class is not None:
                    # selected_voxel = random_choice(voxels_of_that_class, 0)
                    # selected_voxel = voxels_of_that_class[0]
                    bbox_x_lb = tf.maximum(lb_x, selected_voxel[0] - self.basic_generator_patch_size[0] // 2)
                    bbox_y_lb = tf.maximum(lb_y, selected_voxel[1] - self.basic_generator_patch_size[1] // 2)
                    bbox_z_lb = tf.maximum(lb_z, selected_voxel[2] - self.basic_generator_patch_size[2] // 2)
                else:
                    bbox_x_lb = tf.random.uniform([], minval=lb_x, maxval=ub_x+1, dtype=tf.int32)
                    bbox_y_lb = tf.random.uniform([], minval=lb_y, maxval=ub_y+1, dtype=tf.int32)
                    bbox_z_lb = tf.random.uniform([], minval=lb_z, maxval=ub_z+1, dtype=tf.int32)
    ...
    def formalize_data_2d(self, data):
        ...
        for i in range(self.batch_size):
            ...
            if not force_fg:
                ...
                if len(foreground_classes) == 0:
                    selected_class = None
                    random_slice = random_choice(tf.range(tf.shape(case_all_data)[1]), 0)[0]
                    tf.print(f'case does not contain any foreground classes {case_identifier}')
                else:
                    voxels_of_that_class = class_locations[1]
                    rand_val = tf.random.uniform([], minval=0, maxval=len(class_locations), dtype=tf.int32)
                    for key in class_locations.keys():
                        if tf.constant(key, dtype=tf.int32) == rand_val:
                            voxels_of_that_class = class_locations[key]
                            valid_slices, _ = tf.unique(voxels_of_that_class[:, 0])
            ...
            need_to_pad = self.basic_generator_patch_size - self.patch_size
            for d in range(2):
                if tf.shape(need_to_pad)[d] + tf.shape(case_all_data)[d+1] < self.basic_generator_patch_size[d]:
...
class TestSpatialTransform(tf.keras.layers.Layer):
    def __init__(self, do_dummy_2D, patch_size, patch_center_dist_from_border=30,
                ...
                do_scale=True, scale=(0.75, 1.25), border_mode_data='nearest', border_cval_data=0, order_data=3, data_key='images', label_key='labels',
                border_mode_seg='constant', border_cval_seg=0, order_seg=0, random_crop=True, p_el_per_sample=1, p_scale_per_sample=1, p_rot_per_sample=1,
                independent_scale_for_each_axis=False, p_rot_per_axis:float=1, p_independent_scale_per_axis: int=1) -> None:
    ...
    def __call__(self, data):
        ...
        ret_val = augment_spatial(images, segs, patch_size=patch_size,
...
def augment_spatial(data, seg, patch_size, patch_center_dist_from_border=30,
    ...
    def augment_per_sample(sample_id, patch_size, data, seg, data_result, seg_result):
        ...
        if modified_coords:
            ...
            def body_fn(d, coords):
                if random_crop:
                    ctr = tf.random.uniform([], patch_center_dist_from_border[d], tf.cast(tf.shape(data)[d+2], dtype=tf.float32) - patch_center_dist_from_border[d])
                else:
                    ctr = tf.cast(tf.shape(data)[d+2], dtype=tf.float32) / 2. - 0.5
            ...
            _, coords = tf.while_loop(loop_cond, body_fn, [d, coords], shape_invariants=[tf.TensorShape(None), coords.get_shape()])
            data_sample = tf.zeros(tf.shape(data_result)[1:])
            channel_id = tf.constant(0)
            cond_to_loop_data = lambda channel_id, data_sample: tf.less(channel_id, tf.shape(data)[1])
    ...
    if not isinstance(patch_center_dist_from_border, tf.Tensor):
        patch_center_dist_from_border = tf.convert_to_tensor(patch_center_dist_from_border, dtype=tf.float32)
    cond_to_loop = lambda sample_id, patch_size, data, seg, data_result, seg_result: tf.less(sample_id, sample_num)
...
def interpolate_img(img, coords, order=3, mode='nearest', cval=0.0, is_seg=False):
    if is_seg and order != 0:
        ...
        i = tf.constant(0)
        _, _, _, result, _ = tf.while_loop(cond_to_loop, body_fn, [img, i, coords, result, order])
        return result
    else:
...
def map_coordinates_3d(img, coords, order=3):
    ...
    x, y, z = tf.meshgrid(tf.range(tf.shape(img)[0]), tf.range(tf.shape(img)[1]), tf.range(tf.shape(img)[2]), indexing='ij')
    x, y, z = tf.reshape(x, (-1, 1)), tf.reshape(y, (-1, 1)), tf.reshape(z, (-1, 1))
    original_coords = tf.concat([x, y, z], axis=1)
    original_coords = original_coords[tf.newaxis,]
    original_coords = tf.cast(original_coords, tf.float32)
    tmp_img = tf.reshape(img, (1, -1, 1))
    tmp_img = tf.cast(tmp_img, tf.float32)
    result = tfa.image.interpolate_spline(original_coords, tmp_img, new_coords, order=order)
...
def map_coordinates_2d(img, coords, order=3):
    ...
    new_coords = new_coords[tf.newaxis,]
    new_coords = tf.cast(new_coords, tf.float32)
    x, y = tf.meshgrid(tf.range(tf.shape(img)[0]), tf.range(tf.shape(img)[1]), indexing='ij')
...
def map_coordinates(input, coordinates, is_seg=False, result=None, output=None, order=3, mode='constant', cval=0.0, prefilter=True):
...
def spline_filter(input, order=3, output=tf.float32, mode='mirror'):
    if order not in [0, 1]:
        axis = tf.constant(0)
        cond_to_loop = lambda input, order, axis, mode: tf.less(axis, tf.rank(input))
        output, _, _, _ = tf.while_loop(cond_to_loop, spline_filter1d, [input, order, axis, mode])
    else:
        output = input
    return output
...
def prepared_for_spline_filter(input, mode, cval):
    if mode in ['nearest', 'grid-constant']:
        ...
        if mode == 'grid-constant':
            padded = tf.pad(input, tf.ones((tf.rank(input), 2), dtype=tf.int32) * npad, mode='CONSTANT', constant_values=cval)
        elif mode == 'nearest':
            padded = input
            for _ in range(npad):
                padded = tf.pad(padded, tf.ones((tf.rank(padded), 2), dtype=tf.int32), mode="SYMMETRIC")
    else:
        npad = 0
        padded = input
    return padded, npad
...
def crop(data, seg=None, crop_size=128, margins=(0, 0, 0), crop_type="center",
    ...
    def body_fn(b, data_result, seg_result):
        data_shape_here = tf.concat([[data_shape[0]], tf.shape(data[b])], axis=0)
        if seg is not None:
        ...
        need_to_pad_lb =  tf.map_fn(lambda d: tf.abs(tf.minimum(0, lbs[d])), elems=tf.range(dim))
        need_to_pad_ub = tf.map_fn(lambda d: tf.abs(tf.minimum(0, data_shape_here[d+2] - (lbs[d] + crop_size[d]))), elems=tf.range(dim))
        need_to_pad = tf.concat([tf.reshape(need_to_pad_lb, (-1, 1)), tf.reshape(need_to_pad_ub, (-1, 1))], axis=1)
        need_to_pad = tf.concat([tf.constant([[0, 0]]), need_to_pad], axis=0)

        ubs = tf.map_fn(lambda d: tf.minimum(lbs[d] + crop_size[d], data_shape_here[d+2]), elems=tf.range(dim))
        ...
        slicer_data_begin = tf.concat([tf.constant([0]), slicer_data_begin], axis=0)

        slicer_data_size = tf.map_fn(lambda d: ubs[d] - lbs[d], elems=tf.range(dim))
        slicer_data_size = tf.concat([[data_shape_here[1]], slicer_data_size], axis=0)
        ...
        if seg_result is not None:
            ...
            slicer_seg_begin = tf.concat([tf.constant([0]), slicer_seg_begin], axis=0)

            slicer_seg_size = tf.map_fn(lambda d: ubs[d] - lbs[d], elems=tf.range(dim))
            slicer_seg_size = tf.concat([[seg_shape_here[1]], slicer_seg_size], axis=0)
            seg_cropped = tf.slice(seg[b], slicer_seg_begin, slicer_seg_size)

        data_result_b = tf.cond(tf.reduce_any(tf.less(tf.constant(0), need_to_pad)), 
                                            lambda: pad(data_cropped, need_to_pad, pad_mode, pad_kwargs),
                                            lambda: data_cropped)
        seg_result_b = tf.cond(tf.reduce_any(tf.less(tf.constant(0), need_to_pad)), 
    ...
    b = tf.constant(0)
    _, data_return, seg_return = tf.while_loop(cond_to_loop, body_fn, [b, data_return, seg_return])
    return data_return, seg_return

def pad(data, need_to_pad, pad_mode, pad_kwargs):
    return tf.pad(data, need_to_pad, mode=pad_mode, constant_values=pad_kwargs['constant_values'])

def get_lbs_for_center_crop(crop_size, data_shape):
    lbs = tf.map_fn(lambda i: (data_shape[i] - crop_size[i]) // 2, elems=tf.range(tf.shape(data_shape)[0]-2))
    return lbs

def get_lbs_for_random_crop(crop_size, data_shape, margins):
    lbs = tf.map_fn(lambda i: tf.cond(tf.less(margins[i], data_shape[i+2]-crop_size[i]-margins[i]),
                                    lambda: tf.random.uniform([], minval=margins[i], maxval=data_shape[i+2]-crop_size[i]-margins[i], dtype=tf.int32),
                                    lambda: (data_shape[i+2] - crop_size[i]) // 2),
                    elems=tf.range(tf.shape(data_shape)[0]-2))
    return lbs

def create_zero_centered_coordinate_mesh(shape):
    coords = tf.convert_to_tensor(tf.meshgrid(tf.range(shape[0]), tf.range(shape[1]), tf.range(shape[2]), indexing='ij'), dtype=tf.float32)
    shape = tf.cast((shape - 1), dtype=tf.float32) / 2.
    '''
    for d in range(3):
        coords_d = coords[d] - shape[d]
        coords = update_tf_channel(coords, d, coords_d)
    '''
    coords = tf.map_fn(lambda d: coords[d] - shape[d], elems=tf.range(3), dtype=tf.float32)
...
def update_tf_channel(data, idx, update_value):
    update_value = update_value[tf.newaxis,]
