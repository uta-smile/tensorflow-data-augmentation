...
# Local
from tfda.augmentations.pyramid_augmentations import (
    augment_apply_random_binary_operation,
    augment_move_seg_as_onehot_data_batch,
    do_remove_from_origin,
)
...
class RemoveRandomConnectedComponentFromOneHotEncodingTransform(TFDABase):
    ...
    def call(self, data_dict: TFDAData) -> TFDAData:
        ...
        return data_dict.new_data(
            tf.map_fn(lambda b: self.map1(data, b), tf.shape(data)[0])
        )
    ...
    def cond2(self, data, b, c, lab, num_comp, num_voxels) -> tf.Tensor:
        component_sizes = tf.map_fn(
            lambda i: tf.reduce_sum(tf.cast(tf.equal(lab, i), tf.int64)),
            tf.range(1, num_comp + 1),
        )
        component_ids_w0 = tf.map_fn(
            lambda i: tf.cond(
                tf.less(
                    component_sizes[i],
        ...
        length = tf.reduce_sum(
            tf.cast(tf.not_equal(component_ids_w0, 0), tf.int64)
        )
...
class ApplyRandomBinaryOperatorTransform(TFDABase):
    ...
    def call(self, data_dict: TFDAData) -> TFDAData:
        data = data_dict.data
        new_data = tf.map_fn(
            lambda b: augment_apply_random_binary_operation(
                data[b],
                self._channel_idx,
                self.defs.p_per_sample,
                self._strel_size,
                self._p_per_label,
            ),
...
class MoveSegAsOneHotToData(TFDABase):
    def __init__(
        self, channel_idx, all_seg_labels, remove_from_origin=TFbT, **kws
    ):
        super.__init__(**kws)
        self._channel_idx = channel_idx
    ...
    def call(self, data_dict: TFDAData) -> TFDAData:
        ...
        seg_onehot = tf.map_fn(
            lambda b: augment_move_seg_as_onehot_data_batch(
                seg[b][0], self._all_seg_labels
            ),
        ...
        origin = tf.cond(
            self._remove_from_origin,
            lambda: do_remove_from_origin(origin, self._channel_idx),
            lambda: origin,
        )
